...
import es.amartin.twitterstreammetrics.aggregation.io.conversions.{
  AggregationValueReducer,
...
class TwitterMetricKafkaStreamWriter(
  ...
  override def write(stream: KStream[Integer, String]): Unit = {
    stream
      ...
      .groupByKey(
        new Grouped[AggregationKey, AggregationValue](
          "groupByKey",
          new JSONSerde[AggregationKey],
          new JSONSerde[AggregationValue]))
